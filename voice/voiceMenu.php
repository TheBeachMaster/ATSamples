...
//1. Receive POST from AT
  ...
  if($isActive=='1'){
  	//2a. Switch through the DTMF Digits
  	switch ($dtmfDigits) {
	    case "0":
	        //2b. Compose response - talk to sales...
			  ...
			  $response .= '<Dial phoneNumbers="user.username@<country>.sip.africastalking.com" ringbackTone="http://www.myhost.com/file.mp3"/>';
			  $response .= '</Response>';
	    ...
	    case "2":
	        //2d. Redirect to the main IVR...
			  $response  = '<?xml version="1.0" encoding="UTF-8"?>';
			  $response .= '<Response>';
			  $response .= '<Redirect>https://www.myhost.com/voiceCall.php</Redirect>';
			  $response .= '</Response>';
